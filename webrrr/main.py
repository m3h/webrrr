...
class DB:
    ...
    def create_tables(self):
        ...
        cur.execute('''CREATE TABLE IF NOT EXISTS URL(url text primary key, netloc text, last_visited datetime, error text, foreign key (netloc) REFERENCES Robots(netloc))''')
        cur.execute('''CREATE TABLE IF NOT EXISTS Robots(netloc text primary key, robots_txt text)''')
        cur.execute('''
    ...
    def add_urls(self, parent_url, urls):
        ...
        cur = self.con.cursor()
        cur.executemany('''INSERT INTO URL (url, netloc, last_visited)
            SELECT ?, ?, ?
            WHERE NOT EXISTS (
                SELECT 1 FROM URL WHERE url = ?
            )
        ''', [[url, urllib.parse.urlparse(url).netloc, last_visited, url] for url in urls])

        cur.executemany('INSERT INTO Links VALUES (?, ?)', [[parent_url, url] for url in urls])
        self.con.commit()
    ...
    def visit_url(self, url, error_str):
        ...
        cur = self.con.cursor()
        cur.execute('UPDATE URL SET last_visited = ?, error = ? WHERE url = ?', [last_visited, error_str, url])
        self.con.commit()
    ...
    def is_visited(self, url):
        ...
        cur = self.con.cursor()
        cur.execute('SELECT COUNT(1) FROM URL WHERE last_visited is not NULL AND url = ?', [url])
...
class Fetcher:
    ...
    def canonicalize(self, url_part: str, base_url: str)        :
        ...
        if o.scheme == '' and o.netloc == '':
            ...
            return urllib.parse.urlunparse((base_o.scheme, base_o.netloc, o.path, o.params, o.query, fragment))
        else:
            return urllib.parse.urlunparse((o.scheme, o.netloc, o.path, o.params, o.query, fragment))
